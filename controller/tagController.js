...
import { ApiResponse } from "../utils/ApiResponse.js"; // Import your wrapper

export const getAllProblemTags = (req, res) => {
    try {
        // Wrapped in ApiResponse for consistency!
        res.status(200).json(new ApiResponse(200, [...ALLOWED_PROBLEM_TAGS].sort()));
    } catch (error) {
...
export const getAllCompanyTags = (req, res) => {
    try {
        res.status(200).json(new ApiResponse(200, [...ALLOWED_COMPANY_TAGS].sort())); 
    } catch (error) {
